...
- **Framer Motion**: Integrates `framer-motion` for fluid page transitions, card animations, and interactive button effects, enhancing the overall user experience.
...
- **SWR**: A React Hooks library for data fetching, caching, and revalidation, used for efficient data management (e.g., pagination, profile data).
- **Framer Motion**: A production-ready motion library for React, used for animations and transitions.
- **Zustand**: A small, fast, and scalable bear-bones state-management solution, used for global state like reading status counts.
- **date-fns**: A modern JavaScript date utility library.

**Backend (API - separate project):**

- **NestJS**: A progressive Node.js framework for building efficient, reliable, and scalable server-side applications. (Assumed, as per project context)
- **Database**: (e.g., PostgreSQL, MongoDB - specific database not defined in frontend, but implied by API interactions)
- **Cloud Storage**: (e.g., AWS S3 for profile pictures - implied by signed URL functionality)
...
3.  **Set up Environment Variables**:
    ...
    ```
    NEXT_PUBLIC_API_URL=http://localhost:3001 # Or your deployed backend URL
    ```
